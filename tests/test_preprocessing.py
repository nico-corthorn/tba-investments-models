...
import pytest
from tbainvestetl.utils.sql_manager import ManagerSQL

from mediasent.preprocessing import ModelConfig, generate_model_id, load_config, register_model
...
def test_load_config_unit_test():
    ...
    mock_config = {
        "model": {
            ...
            "variant_version": "001",
            "hyperparameters": {"temperature": 0.7},
            "prompt_template_path": "mediasent/config/prompt_template.txt",
            "description": "Test model",
        }
    }

    with patch("tbainvestetl.utils.aws.get_s3_file") as mock_get_s3_file:
        mock_get_s3_file.return_value = json.dumps(mock_config)
        config = load_config()
        assert config["model"]["base_model"] == "llama2"
        assert config["model"]["prompt_template_path"] == "mediasent/config/prompt_template.txt"
        mock_get_s3_file.assert_called_once_with("mediasent/config/model_config.json")
...
def ntest_register_model_integration(sql):
    ...
    mock_config_1 = {
        "model": {
            ...
            "prompt_template_file": "mediasent/config/prompt_template.txt",
            "description": "Test model",
        }
    }
    mock_config_2 = {
